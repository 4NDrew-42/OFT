...
export const OrionVectorSearch: React.FC<OrionVectorSearchProps> = ({
  ...
  // Simple debug on component mount
  console.log('🔍 ORION SEARCH: Component initialized with API_BASE_URL:', API_BASE_URL);
  ...
  const performVectorSearch = useCallback(async (
    ...
    setIsSearching(true);
    console.log('🔍 ORION SEARCH: Searching for:', searchQuery);
    ...
    try {
      ...
      if (!response.ok) {
        console.error('🔍 ORION SEARCH: API Error', response.status, response.statusText);
      ...
      if (!data.success) {
        console.error('🔍 ORION SEARCH: Search failed', data.error);
        throw new Error(data.error || 'Search failed');
      }

      const searchResults: SearchResult[] = data.results.map((result: any) => transformToSearchResult(result));
      console.log('🔍 ORION SEARCH: Found', searchResults.length, 'results');
      ...
      try {
        ...
        console.warn('🔍 ORION SEARCH: Analytics tracking failed', analyticsError);
      }
      ...
      console.error('🔍 ORION SEARCH: Error', error instanceof Error ? error.message : 'Unknown error');
      return [];
    } finally {
